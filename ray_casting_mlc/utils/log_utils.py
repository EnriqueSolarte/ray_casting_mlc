...
def update_log_results(log_results, epoch_results):
    ...
    if new_best_model:
        ...
        logging.info(f"New best model found: 2DIoU: {log_results['best']['2DIoU']:0.4f} - 3DIoU: {log_results['best']['3DIoU']:0.4f}")
        return True
    logging.info(f"Best model: 2DIoU: {log_results['best']['2DIoU']:0.4f} - 3DIoU: {log_results['best']['3DIoU']:0.4f}")
    return False
